...
네이버 블로그 `hyunjp88`의 `공지사항(categoryNo=33)` 카테고리 글과 블로그에서 공지로 지정한 글을 원본으로 사용합니다. RSS와 블로그 공지 목록에서 동기화 대상을 찾고, 개별 글 페이지에서 최신 제목·발행일·전체 본문과 이미지를 가져옵니다. 네이버 글 번호로 중복을 방지하고 본문 해시로 수정 여부를 판별합니다.
